...
function makeparameters(load_profile, gen_profile)
...
function makemodel(load_profile, gen_profile)
    model = Model(HiGHS.Optimizer)

    params = makeparameters(load_profile, gen_profile)
...
function runmodel()
    ...
    for load_p in load_profiles
        for gen_p in gen_profiles
            println("Running: load=$(load_p)  |  gen=$(gen_p)")
            ...
            run_counter += 1
            run_label = "run_$(run_counter)_load$(load_p)_gen$(gen_p)"
